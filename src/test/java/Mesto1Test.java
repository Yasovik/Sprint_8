...
public class Mesto1Test {
    String bearerToken = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJfaWQiOiI2M2M3ZWMyNjQ0YzhkNTAwM2QwMjI2ZmYiLCJpYXQiOjE2Nzk3NTU0MTMsImV4cCI6MTY4MDM2MDIxM30.R4lrMrIktneQgh_i4FQ5kT1nL4n2wYzjqm6I5HQSAbQ";
  ...
  //      given()
    //           .auth().oauth2(bearerToken) // Передаём токен для аутентификации
      //          .get("/api/users/me") // Делаем GET-запрос
       //         .then().assertThat().body("data.name", equalTo("data.name")); // Проверяем, что имя соответствует ожидаемому
  //  }
}
